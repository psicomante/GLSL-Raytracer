 ...
 * @type {vec3} center
 * @type {vec3} radius
 * @type {vec3} color diffuse color of the plane
 */
struct Plane
{
  vec3 p;
 ...
 *
 * @type {float} t
 * @type {vec3} point intersection point
 * @type {vec3} normal normal to the intersection point
 * @type {int} hit number
 * @type {vec3} color computed color
 */
struct Intersection {
  float t;
  vec3 point;
...
void raySphereIntersection(Ray ray, Sphere sphere, inout Intersection isect) {
  ...
  if ( (t >= 0.0) && (t < isect.t) ) {
    ...
    // intersection point
    vec3 point = vec3(ray.origin.x + ray.direction.x * t,
 ...
 *
 */
void rayPlaneIntersection(Ray ray, Plane pl, inout Intersection isect) {
  // d = -(p . n)
  // t = -(ray.org . n + d) / (ray.dir . n)
  float d = -dot(pl.p, pl.n);
  float v = dot(ray.direction, pl.n);

  if (abs(v) < 1.0e-6)
    return; // the plane is parallel to the ray.

  float t = -(dot(ray.origin, pl.n) + d) / v;
  ...
  if ( (t > 0.0) && (t < isect.t) ) {
    ...
    vec3 p = vec3(ray.origin.x + t * ray.direction.x,
    ...
    ||  (mod(dp.x, 1.0) < 0.5 && mod(dp.z, 1.0) < 0.5)){
      ...
      isect.color = vec4(pl.color.rgb * 0.5, 1.0);
    }

    // isect.point = vec3(0.);
    // isect.normal = vec3(0., 1.0, 0.0);
...
void Intersect(Ray r, inout Intersection i) {
  ...
  rayPlaneIntersection(r, plane, i);
}
...
void main() {
  ...
  // sphere details
  spheres[0].center = vec3(sin(time)*2. - 2.5, 0.0, cos(time)*2. - 2.5);
  spheres[0].radius = 0.5;
  spheres[0].color = vec4(1,0,0,1);
  spheres[1].center = vec3(-0.7, 0.0, -3.0);
  spheres[1].radius = 1.5;
  spheres[1].color = vec4(0,1,0,1);
  spheres[2].center = vec3(sin(time)*2. - 0.5, sin(time), cos(time)*2. - 1.5);
  ...
  if (i.hit != 0){
    float eps  = 0.0001;
    Ray rsh;
    rsh.origin = i.point + i.normal * eps;// 法線方向にすこしずらす
    rsh.direction = light;
    Intersection ish;
    ish.hit = 0;
    ish.t = 1.0e+30;
    ish.normal = ish.point = ish.color.rgb = vec3(0, 0, 0);
    Intersect(rsh, ish);

    float shadow = 1.0 - float(ish.hit);
    // direction
    pixel_color.rgb *= dot(i.normal, light);
    pixel_color.rgb = max(0.0,dot(i.normal, light)) * i.color.rgb * shadow;
  }
