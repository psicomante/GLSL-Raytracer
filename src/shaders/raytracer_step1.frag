...
vec4 sphere_color = vec4(1,0,0,1);

// background
vec4 bg = vec4(0,0,0,1);

/**
 * Structure of a normalized vector
 *
 * Commonly
...
void main() {
  ...
  // initialize color as the background value
  vec4 pixel_color = bg;

  // intersection test
  float hit = raySphereIntersection(ray, sphere);

  // ray - sphere intersection gave a positive result
  if (hit > 0.) {
    // set the pixel color as the sphere's one
    pixel_color = sphere.color * vec4(vec3(2.0 - hit / 2.0),1.);
  }
