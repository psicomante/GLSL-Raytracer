...
void main() {
  ...
  spheres[0].center = vec3(cos(time)*2., 0.0, sin(time)*2. - 0.5);
  spheres[0].radius = 0.5;
  spheres[0].color = vec4(1,0,0,1);
  spheres[1].center = vec3(-0.5, 0.0, -3.0);
  spheres[1].radius = 1.5;
  spheres[1].color = vec4(0,1,0,1);
  spheres[2].center = vec3(cos(time)*2. + 2.0, 0.0, sin(time)*2. -1.5);
  ...
  vec4 pixel_color = i.color * (2.0 - i.t / 2.0);
